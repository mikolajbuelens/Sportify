    ...
    const getSavedSteps = async () => {
      ...
      if (Array.isArray(savedSteps)) {
        const filteredSteps = savedSteps.filter((step) => {
          ...
          return (
            stepDate.getDate() === today.getDate() &&
            stepDate.getMonth() === today.getMonth() &&
            stepDate.getFullYear() === today.getFullYear()
          );
        });
        et;
        const totalSteps = filteredSteps.reduce(
          (accumulator, currentValue) => accumulator + currentValue.stepCount,
          0
        );
              ...
              <View style={styles.goal}>
                ...
                <View style={styles.goalkm}>
                  ...
                  <Text style={styles.label}>
                    Km ({((goal * 100000) / 76).toFixed(0)} steps){" "}
                  </Text>
                </View>
              </View>
